...
$.fn.html5uploader = function(opts){
	...
	var defaults = {
		fileTypeExts:'image/gif',//允许上传的文件类型，填写mime类型
		url:'',//文件提交的地址
		auto:false,//自动上传
		multi:true,//默认允许选择多个文件
		buttonText:'选择文件',//上传按钮上的文字
		removeTimeout: 1000,//上传完成后进度条的消失时间
//		itemTemplate:'<li id="${fileID}file"><div class="progress"><div class="progressbar"></div></div><span class="filename">${fileName}</span><span class="progressnum">0/${fileSize}</span><a class="delfilebtn">删除</a></li>',//上传队列显示的模板,最外层标签使用<li>
        itemTemplate:'<input id="${fileID}" name="newfile" size=${fileSize} type="file" />',
	...
	this.each(function(){
		...
		var uploadFileList = $('<div class="filelist"></div>');
		...
		//创建文件对象
              ...
              console.log(uploadFileList.html());
				 //如果配置非自动上传，绑定上传事件
				 if(!option.auto){
