...
- (void)handleRequest
{
	...
	if ([method isEqualToString:@"GET"])
	{
		...
		{
			NSMutableArray *segs = [NSMutableArray arrayWithArray:[path componentsSeparatedByString:@"/"]];
			if ([segs count] >= 2)
			{
                [segs removeObject:@""];
                [segs removeObject:@"files"];
                NSString *fullPath = [[FileManager docPath] stringByAppendingPathComponent:[segs componentsJoinedByString:@"/"]];
                if ([FileManager isDirPath:fullPath]) {
                    [self actionListPath:fullPath];
                } else {
                    [self actionShow:fullPath];
                }
...
- (void)actionList
{
	...
	for(int i = 0; i<[delegate numberOfFiles]; ++i)
	{
        ...
        BOOL isDir = [FileManager isDirPath:filePath];
        NSString *type = isDir ? @"dir" : @"file";
        
        NSString* file = [filename stringByReplacingOccurrencesOfString:@"'" withString:@"\\'"] ;
		[output appendFormat:@"{\"name\":\"%@\", \"id\":\"%d\", \"type\":\"%@\"},", file, i, type];
	}
	if ([output length] > 1)
	{
...
- (void)actionListPath:(NSString *)path
{
	if (delegate == nil)
	{
		[connection handleResourceNotFound];
	...
	NSMutableString *output = [[NSMutableString alloc] init];
	[output appendString:@"["];
    [delegate loadFileListOfPath:path];
	for(int i = 0; i<[delegate numberOfFiles]; ++i)
	{
		NSString* filename = [delegate fileNameAtIndex:i];

        NSString* filePath = [path stringByAppendingPathComponent:[filename stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding] ];
        BOOL isDir = [FileManager isDirPath:filePath];
        NSString *type = isDir ? @"dir" : @"file";
...
- (void)actionShow:(NSString*)filePath
{
	if (delegate == nil)
	{
		[connection handleResourceNotFound];
...
//	NSString* filePath = [delegate filePathForFileName:[fileName stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding] ];
	...
	HTTPFileResponse* response = [[[HTTPFileResponse alloc] initWithFilePath:[filePath stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] autorelease];
