...
- (NSString*)hostName
{
	...
	if (error) {
		NSLog(@"%s %s", __func__, gai_strerror(error));
	} else {
        cursor = addrs;
        while (cursor != NULL)
        {
            if (cursor->ifa_addr->sa_family == AF_INET && (cursor->ifa_flags & IFF_LOOPBACK) == 0)
            {
                ...
                if (
                    [ifa_name isEqualToString:@"en0"]
#if (TARGET_IPHONE_SIMULATOR)
                    || [ifa_name isEqualToString:@"en1"]
#endif
                    )
                {
                    hostname = [NSString stringWithUTF8String:inet_ntoa(((struct sockaddr_in *)cursor->ifa_addr)->sin_addr)];
                    NSLog(@"%s hostname:%@ on interface:%@", __func__, hostname, ifa_name);
                    break;
                }
            }
...
- (BOOL)start:(NSError **)error
{
	...
	{
		NSLog(@"Failed to start HTTP Server: %@", (*error).localizedDescription);
	}
