...
- (void)handleHTTPRequestBody:(NSData*)data tag:(long)tag
{
    ...
    // post the notification only when uploading progress changed more than 1%
    if (tag == HTTP_REQUEST_BODY_MULTIPART) {
        static float lastPercent;
        float percent = 1.0 - remain * 1.0 / bodyLength;
        if (ABS(percent - lastPercent) > 0.01 || percent >= 1.0) {
            lastPercent = percent;
            NSString* progress = [NSString stringWithFormat:@"%f", percent];
            [[NSNotificationCenter defaultCenter] postNotificationName:HTTPUploadingProgressNotification object:self userInfo:[NSDictionary dictionaryWithObject:progress forKey:@"progress"]];
        }
    }
...
- (void)handleMultipartHeader:(NSData*)body
{
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:HTTPUploadingStartNotification object:self userInfo:@{@"fileName":filename}];
